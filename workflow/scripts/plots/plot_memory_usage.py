...
for i in range(4, 401):
    ...
    if os.path.exists(pyr_path):
        ...
        if "alg_only_mem" in pyr_df.columns and not pyr_df.empty:
            pyr_times.append(pyr_df["alg_only_mem"].iloc[0])
        else:
            pyr_times.append(None) 
    else:
        pyr_times.append(None)  
    ...
    if os.path.exists(cx_path):
        ...
        if "alg_only_mem" in cx_df.columns and not cx_df.empty:
            cx_times.append(cx_df["alg_only_mem"].iloc[0])
        else:
            cx_times.append(None) 
    else:
        cx_times.append(None)  
...
for i in range(5, 401, 5):
    ...
    if os.path.exists(cx_path):
        cx_df = pd.read_csv(cx_path)
        if "alg_only_mem" in cx_df.columns and not cx_df.empty:
            cx_l11_times.append(cx_df["alg_only_mem"].iloc[0])
        else:
...
for i in range(5, 401, 5):
    ...
    if os.path.exists(cx_path):
        cx_df = pd.read_csv(cx_path)
        if "alg_only_mem" in cx_df.columns and not cx_df.empty:
            cx_l12_times.append(cx_df["alg_only_mem"].iloc[0])
        else:
